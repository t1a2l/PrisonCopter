...
namespace PrisonHelicopter.Utils
{
    ...
    {
        private const string HarmonyId = "github.com/t1a2l/PrisonCopter";
            ...
            MethodDefinition transpiler = null)
        {
            ...
            {
                throw new Exception(
                    $"PrisonHelicopter: prefix, postfix and transpiler are null for method {original.Type.FullName}.{original.MethodName}");
            }

            try
            {
                Debug.Log($"PrisonHelicopter: Patching method {original.Type.FullName}.{original.MethodName}");
        ...
        private static MethodInfo GetOriginal(MethodDefinition original)
        {
            ...
            {
                throw new Exception(
                    $"PrisonHelicopter: Failed to find original method {original.Type.FullName}.{original.MethodName}");
            }
        ...
        private static MethodInfo GetPatch(MethodDefinition patch)
        {
            ...
            if (methodInfo == null)
            {
                throw new Exception($"PrisonHelicopter: Failed to find patch method {patch.Type.FullName}.{patch.MethodName}");
            }
