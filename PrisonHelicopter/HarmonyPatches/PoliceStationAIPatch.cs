...
namespace PrisonHelicopter.HarmonyPatches {
        ...
        public static bool StartTransfer(PoliceStationAI __instance, ushort buildingID, ref Building data, TransferManager.TransferReason material, TransferManager.TransferOffer offer) {
            if (material == TransferManager.TransferReason.Crime || material == TransferManager.TransferReason.CriminalMove || material == (TransferManager.TransferReason)126) {
                ...
                if(police_building_info.GetAI() is PoliceStationAI policeStationAI1 &&
                   ...
                   (data.m_flags & Building.Flags.Downgrading) == 0 &&
                   material == (TransferManager.TransferReason)126)
                {
                    vehicle_level = ItemClass.Level.Level4;
                }
                VehicleInfo randomVehicleInfo = Singleton<VehicleManager>.instance.GetRandomVehicleInfo(ref Singleton<SimulationManager>.instance.m_randomizer, __instance.m_info.m_class.m_service, __instance.m_info.m_class.m_subService, vehicle_level, VehicleInfo.VehicleType.Car);
                if (randomVehicleInfo != null) {
                    ...
                    if (material == TransferManager.TransferReason.CriminalMove && randomVehicleInfo.m_vehicleType == VehicleInfo.VehicleType.Helicopter) {
                        // ignore small police stations
                        if(police_building_info.GetAI() is PoliceStationAI policeStationAI && police_building_info.m_class.m_level < ItemClass.Level.Level4 && (data.m_flags & Building.Flags.Downgrading) != 0)
                        {
                            return false;
                        }
        ...
        public static bool GetLocalizedStats(PoliceStationAI __instance, ushort buildingID, ref Building data, ref string __result)
	{
		...
		CalculateOwnVehicles(__instance, buildingID, ref data, TransferManager.TransferReason.CriminalMove, ref count, ref cargo, ref capacity, ref outside);
            ...
            else if(__instance.m_info.m_class.m_level < ItemClass.Level.Level4 && (data.m_flags & Building.Flags.Downgrading) == 0)
            {
	    ...
	    if (__instance.m_info.m_class.m_level >= ItemClass.Level.Level4)
	    {
            ...
            else if(__instance.m_info.m_class.m_level < ItemClass.Level.Level4 && (data.m_flags & Building.Flags.Downgrading) == 0)
            {
                ...
                __result = text + LocaleFormatter.FormatGeneric("AIINFO_PRISON_CARS", count1, num5);
            }
            else
            {
                text = LocaleFormatter.FormatGeneric("AIINFO_POLICESTATION_CRIMINALS", num3, __instance.JailCapacity) + Environment.NewLine;
        ...
        public static bool ProduceGoods(PoliceStationAI __instance, ushort buildingID, ref Building buildingData, ref Building.Frame frameData, int productionRate, int finalProductionRate, ref Citizen.BehaviourData behaviour, int aliveWorkerCount, int totalWorkerCount, int workPlaceCount, int aliveVisitorCount, int totalVisitorCount, int visitPlaceCount)
	{
	    ...
	    {
                CalculateOwnVehicles(__instance, buildingID, ref buildingData, (TransferManager.TransferReason)126, ref count2, ref cargo2, ref capacity2, ref outside2); // own police vans
		CalculateOwnVehicles(__instance, buildingID, ref buildingData, TransferManager.TransferReason.CriminalMove, ref count3, ref cargo3, ref capacity3, ref outside3); // own prison helicopters
		...
		instance.m_districts.m_buffer[district].m_productionData.m_tempCriminalAmount += (uint)cargo3;
            ...
            else if (__instance.m_info.m_class.m_level < ItemClass.Level.Level4 && (buildingData.m_flags & Building.Flags.Downgrading) == 0) // big police station
            {
                CalculateOwnVehicles(__instance, buildingID, ref buildingData, TransferManager.TransferReason.Crime, ref count, ref cargo, ref capacity, ref outside); // own police cars
                CalculateOwnVehicles(__instance, buildingID, ref buildingData, (TransferManager.TransferReason)126, ref count2, ref cargo2, ref capacity2, ref outside2); // own police vans
                cargo2 = Mathf.Max(0, Mathf.Min(__instance.JailCapacity - num8, cargo2));
		instance.m_districts.m_buffer[district].m_productionData.m_tempCriminalAmount += (uint)cargo2;
		CalculateGuestVehiclesCommonBuildingAI(__instance, buildingID, ref buildingData, TransferManager.TransferReason.CriminalMove, ref count3, ref cargo3, ref capacity3, ref outside3); // guest helicopters or prison vans from prison
            }
	    else // small police station
	    {
		CalculateOwnVehicles(__instance, buildingID, ref buildingData, TransferManager.TransferReason.Crime, ref count, ref cargo, ref capacity, ref outside); // own police cars
                CalculateGuestVehiclesCommonBuildingAI(__instance, buildingID, ref buildingData, (TransferManager.TransferReason)126, ref count2, ref cargo2, ref capacity2, ref outside2); // guest police vans
		CalculateGuestVehiclesCommonBuildingAI(__instance, buildingID, ref buildingData, TransferManager.TransferReason.CriminalMove, ref count3, ref cargo3, ref capacity3, ref outside3); // guest prison vans
	    }
	    int num10 = (finalProductionRate * __instance.PoliceCarCount + 99) / 100;
            int num11 = num10 / 5;
	    if (__instance.m_info.m_class.m_level >= ItemClass.Level.Level4)
	    {
		if (count3 < num10 && capacity3 + num7 <= __instance.JailCapacity - 20)
		{
		    TransferManager.TransferOffer offer4 = default(TransferManager.TransferOffer);
		    offer4.Priority = 2 - count3;
		...
		return false;
            ...
            if (__instance.m_info.m_class.m_level < ItemClass.Level.Level4 && (buildingData.m_flags & Building.Flags.Downgrading) == 0)
	    {
                if (count2 < num11 && capacity2 + num7 <= __instance.JailCapacity - 20)
                {
                    TransferManager.TransferOffer offer2 = default(TransferManager.TransferOffer);
		    offer2.Priority = 2 - count2;
		    offer2.Building = buildingID;
