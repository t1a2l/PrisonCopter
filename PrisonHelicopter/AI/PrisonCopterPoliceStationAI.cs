...
namespace PrisonHelicopter.AI {
        ...
        public override void BuildingDeactivated(ushort buildingID, ref Building data)
	{
	    ...
	    if (m_info.m_class.m_level >= ItemClass.Level.Level4) // prison
	    {
            ...
            else if (m_info.m_class.m_level < ItemClass.Level.Level4 && (building.m_flags & Building.Flags.Downgrading) != 0) // police station with prison vans
            {
                ...
                Singleton<TransferManager>.instance.RemoveIncomingOffer((TransferManager.TransferReason)121, offer); // send prison helicopters from police helicopter depot
	    ...
	    {
		Singleton<TransferManager>.instance.RemoveIncomingOffer(TransferManager.TransferReason.Crime, offer); // send police cars from police station
                Singleton<TransferManager>.instance.RemoveOutgoingOffer((TransferManager.TransferReason)120, offer); // ask for prison vans from police station
                Singleton<TransferManager>.instance.RemoveOutgoingOffer((TransferManager.TransferReason)121, offer); // ask for prison helicopters from police helicopter depot
	    }
        ...
        {
            if (material == TransferManager.TransferReason.Crime || material == (TransferManager.TransferReason)120 || material == TransferManager.TransferReason.CriminalMove)
            {
        ...
        protected override void ProduceGoods(ushort buildingID, ref Building buildingData, ref Building.Frame frameData, int productionRate, int finalProductionRate, ref Citizen.BehaviourData behaviour, int aliveWorkerCount, int totalWorkerCount, int workPlaceCount, int aliveVisitorCount, int totalVisitorCount, int visitPlaceCount)
	{
		...
		CalculateGuestVehicles(buildingID, ref buildingData, (TransferManager.TransferReason)122, ref count3, ref cargo3, ref capacity3, ref outside3); // guest prison helicopters
		...
		instance.m_districts.m_buffer[district].m_productionData.m_tempCriminalAmount += (uint)cargo2;
                CalculateGuestVehicles(buildingID, ref buildingData, (TransferManager.TransferReason)121, ref count3, ref cargo3, ref capacity3, ref outside3); // guest prison helicopters
	    ...
	    {
		CalculateOwnVehicles(buildingID, ref buildingData, TransferManager.TransferReason.Crime, ref count, ref cargo, ref capacity, ref outside); // own police cars
                CalculateGuestVehicles(buildingID, ref buildingData, (TransferManager.TransferReason)120, ref count2, ref cargo2, ref capacity2, ref outside2); // guest prison vans from police station
                CalculateGuestVehicles(buildingID, ref buildingData, TransferManager.TransferReason.CriminalMove, ref count4, ref cargo4, ref capacity4, ref outside4); // guest prison vans from prison
                m_jailOccupancy = num8;
            }
		...
		if (count4 < num10 && capacity4 + num7 <= JailCapacity - 20)
		{
		    ...
		    offer3.Amount = 1;
		    offer3.Active = false;
                    Singleton<TransferManager>.instance.AddOutgoingOffer((TransferManager.TransferReason)122, offer3); 
                }
		return;
	    }
            if (m_info.m_class.m_level < ItemClass.Level.Level4 && (buildingData.m_flags & Building.Flags.Downgrading) != 0) // big police station
	    {
		    ...
		    offer2.Amount = 1;
		    offer2.Active = true;
		    Singleton<TransferManager>.instance.AddIncomingOffer((TransferManager.TransferReason)120, offer2);
                }
	    }
	    if (count < num10)
	    {
	    ...
	    if (num8 >= (JailCapacity * PrisonHelicopterMod.PriosnersPercentage / 100)) // check if prisoner count is above or equal percentage option
	    {
		        ...
		        offer3.Amount = 1;
		        offer3.Active = false;
                        Singleton<TransferManager>.instance.AddOutgoingOffer((TransferManager.TransferReason)121, offer3);
                    }
		        ...
		        offer2.Amount = 1;
		        offer2.Active = false;
                        Singleton<TransferManager>.instance.AddOutgoingOffer((TransferManager.TransferReason)120, offer2);
                    }
