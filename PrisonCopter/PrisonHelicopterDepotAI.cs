...
namespace PrisonCopter {
    ...
    public static class PrisonHelicopterDepotAI {
        ...
        [HarmonyPrefix]
        private static bool ProduceGoods(HelicopterDepotAI __instance, ushort buildingID, ref Building buildingData, ref Building.Frame frameData, int productionRate, int finalProductionRate, ref Citizen.BehaviourData behaviour, int aliveWorkerCount, int totalWorkerCount, int workPlaceCount, int aliveVisitorCount, int totalVisitorCount, int visitPlaceCount) {
            ...
            while (num6 != 0) {
                ...
                if (transferType == transferReason || (transferType == transferReason2 && transferReason2 != TransferManager.TransferReason.None)) {
                    ...
                    if (transferType == TransferManager.TransferReason.CriminalMove & num15 == 0) {
                        num15 = size;
                        num16 = max;
                    }
                    num3++;
            ...
            if (num3 < num2) {
                ...
                var loadedBuildingInfoCount = bInstance.m_buildingCount;
                for (uint i = 0; i < loadedBuildingInfoCount; i++) {
                    var bi = bInstance.m_buildings.m_buffer[i].Info;
                ...
                if (flag2 && transferReason != TransferManager.TransferReason.CriminalMove) {
                    TransferManager.TransferOffer offer = default(TransferManager.TransferOffer);
                    offer.Priority = 6;
                    offer.Building = buildingID;
                    offer.Position = buildingData.m_position;
        ...
        public static bool GetLocalizedStats(HelicopterDepotAI __instance, ushort buildingID, ref Building data, ref string __result)
	{
		...
		if (transferReason2 != TransferManager.TransferReason.None)
		{
			CalculateOwnVehicles(__instance, buildingID, ref data, transferReason2, ref count, ref cargo, ref capacity, ref outside);
                ...
                if(transferReason == TransferManager.TransferReason.Crime || transferReason == TransferManager.TransferReason.CriminalMove)
                {
                    __result = "Police " + __result;
                    __result += Environment.NewLine;
                    __result = "Prison " + __result;
                }
		return false;
	}
    }
