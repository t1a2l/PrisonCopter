...
namespace PrisonHelicopter.Utils
{
    public static class TranspilerUtil
    {
        ...
        internal static MethodInfo DeclaredMethod<TDelegate>(Type type, string name, bool instance = false) where TDelegate : Delegate
        {
            ...
            if (ret == null) LogHelper.Error($"failed to retrieve method {type}.{name}({args.ToSTR()})");
            return ret;
        }
        ...
        public static TDelegate CreateDelegate<TDelegate>(Type type, string name, bool instance) where TDelegate : Delegate
        {
            ...
            var ret = type.GetMethod(name, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null, types, []);
            if (ret == null) LogHelper.Error($"failed to retrieve method {type}.{name}({types.ToSTR()})");
